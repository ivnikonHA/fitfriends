...
import { buyTrainingAction } from '@fitfriends/store';
...
export function PopupBuy({ training, onClose }:PopupBuyProps) {
  const dispatch = useAppDispatch();
  const [qty, setQty] = useState(1);
  const onBuy = async () => {
    if(training.id) {
      dispatch(buyTrainingAction({
        ...
        <div className="popup__wrapper">
          ...
          <div className="popup__content popup__content--purchases">
              ...
              <div className="popup__product-quantity">
                ...
                <div className="input-quantity">
                  ...
                  <div className="input-quantity__input">
                    <label>
                      <input type="text" value={qty} size={2} readOnly />
                    </label>
                  </div>
                  <button
                    className="btn-icon btn-icon--quantity"
                    type="button"
                    aria-label="plus"
                    onClick={() => setQty((prev) => prev === 9 ? prev : prev + 1)}
                  >
                    <svg width={12} height={12} aria-hidden="true">
            ...
            <div className="popup__total">
              ...
              <p className="popup__total-price">{training.price * qty}&nbsp;₽</p>
            </div>
            <div className="popup__button">
              <button className="btn" type="button" onClick={onBuy}>Купить</button>
            </div>
          </div>
        </div>
      </section>
